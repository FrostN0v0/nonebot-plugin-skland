...
- **明日方舟（Arknights）**：角色信息卡片、每日签到、肉鸽战绩与单局详情、抽卡记录查询、从小黑盒导入抽卡记录、干员盒与图鉴查询。
...
│   │   ├── operators.py # 干员筛选、卡片视图与盒子/图鉴组合模型
...
skland import <url>
skland box [target] [--book] [-r <rarity>] [-p <profession>] [-b <branch>] [--position <position>] [--gender <gender>] [-f <faction>] [--race <race>] [-n <name>]
skland efcard [target] [-a] [-s]
skland efgacha [target] [-u] [-b <begin>] [-l <limit>]
```

内置快捷指令在 `hook.py` 启动时注册，并通过 `nonebot_plugin_alconna.command_manager` 持久化到插件缓存目录的 `shortcut.db`。当前包括：森空岛绑定、扫码绑定、森空岛解绑、明日方舟签到、签到详情、全体签到、全体签到详情、各肉鸽主题、角色更新、全体角色更新、资源更新、战绩详情、收藏战绩详情、方舟抽卡记录、导入抽卡记录、干员盒、图鉴、终末地签到、终末地签到详情、终末地全体签到、终末地全体签到详情、`ef|zmd`、终末地抽卡记录、终末地抽卡更新。
...
- `roster_render_max`: 干员盒/图鉴单图渲染干员数量上限，默认 16。
...
- `commands/box.py`：统一处理 `skland box`（仅已拥有）与 `skland box --book`（全图鉴，未拥有灰显）；结果按 `roster_render_max` 分块并发渲染，QQClient 使用合并转发，其他平台逐图发送。
- `schemas/arknights/game_data.py`：`OperatorCatalog` 从官方 `character_table.json`、`char_patch_table.json`、`uniequip_table.json`、`handbook_info_table.json`、`handbook_team_table.json` 构造稳定目录；`OperatorMetadataSnapshot` 定义 PRTS 本地增强数据。
- `schemas/arknights/operators.py`：`OperatorFilter`、`OperatorCard`、`OperatorModule`、`OperatorRoster` 负责筛选语义、持有数据组合与模板视图属性；同一筛选维度为 OR，不同维度为 AND。
- `filters.py`：统一提供立绘、技能、潜能、精英阶段、职业、稀有度、模组及 Half 卡片装饰资源 URL，角色卡与干员盒共同复用。
- 官方数据提供职业、职业分支 ID、部署位置、势力、标签、技能、模组和实装顺序；PRTS 快照以 `char_id` 左连接补充职业分支中文名、性别和种族，阿米娅各职业形态保持独立身份。
- 立绘使用 `char_skin/portrait/{skinId}`，技能使用 `char_skill/{skillId}`，模组使用小写 `uniequip_direction/{typeIcon}`；图鉴未拥有用默认 `#1` 并整体灰显，已拥有用玩家 `skinId`。
- `render.render_operator_roster()` 接收单个 `OperatorRoster` 组合模型，固定 706px Playwright 视口并调用 htmlrender 原生 `template_to_pic()`；页面高度由内容自动撑开，设备缩放为 1.5，截图超时由 `roster_render_timeout` 配置。
- `operator_roster.html.jinja2` 维护 706px、4 列固定网格；筛选标签支持自动换行。页面骨架留在主模板，玩家头部、筛选标签、干员卡、技能和模组槽位拆在 `operator_roster_macros.html.jinja2`，样式编译到 `resources/templates/index.css`。
...
- `tests/test_operator_roster.py` 覆盖官方目录与 PRTS 元数据合并、分页和原子缓存、组合筛选、统一 `--book` 命令、技能/模组组合、分页发送与渲染参数。
