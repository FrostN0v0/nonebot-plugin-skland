...
所有按角色执行的入口均支持 `-r` / `--role`：角色卡片、两游戏抽卡查询、抽卡导入、方舟干员、肉鸽战绩与详情、两游戏个人签到与签到状态。例：`sk gacha -r 2`、`sk efgacha -r 2`、`sk rogue -r 2`。绑定、解绑、账号管理、资源同步及超管全体签到不按单个游戏角色执行。
...
抽卡、导入、干员、肉鸽、战绩详情及个人签到/状态的中文快捷指令同样支持追加 `-r <序号>`；例如 `方舟抽卡记录 -r 2`、`终末地抽卡记录 -r 2`、`树海肉鸽 -r 2`。
